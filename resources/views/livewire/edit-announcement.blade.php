<div>
    <x-success />
    <form wire:submit.prevent="updateAnnouncement" class="mb-5 button-1 text-light  p-5 radius-carousel" enctype="multipart/form-data">
        @csrf
        <div class="row g-3">
            ...
            @if($storageImages->isNotEmpty() || !empty($images) )
                <div class="col-12">
                    <p>Photo preview:</p>
                    <div class="row border border-4 text-center rounded shadow py-4">
                        
                        @foreach($storageImages as $key => $image)
                        <div class="col my-3">
                            <img style="width: 200px;" src="{{$image->getUrl(1200, 1200)}}" class="img-preview mx-auto shadow rounded"></img>
                            <button type="button" class="btn btn-danger shadow d-block text-center mt-2 mx-auto" wire:click="removeStorageImage({{$key}})">{{__('ui.elimina')}}</button>
                        </div>
                        @endforeach
                        @if(!empty($images))
                            @foreach($images as $key => $image)
                                <div class="col my-3">
                                    <img style="width: 200px;" src="{{$image->temporaryUrl()}}" class="img-preview mx-auto shadow rounded"></img>
                                    <button type="button" class="btn btn-danger shadow d-block text-center mt-2 mx-auto" wire:click="removeImage({{$key}})">{{__('ui.elimina')}}</button>
                                </div>
                            @endforeach
                        @endif
            ...
            <!-- Bottone -->
            <div class="col-12">
                <button type="submit" class="btn logo-bg fw-semibold mt-2 mb-3 text-nav-custom2 btn-fabio" wire:ignore >{{__('ui.modifica')}}</button>

                <button class="btn logo-bg fw-semibold mt-2 mb-3 text-nav-custom2 btn-fabio" wire:ignore wire:click="deleteAnnouncement({{$editAnnouncement}})">{{__('ui.elimina')}}</button>
